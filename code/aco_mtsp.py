...
def runACOImpl(tsp, depotIndex, nSalesmen):
    #problem = tsplib95.load_problem('bays29.tsp')
    G = tsp.get_graph() #problem.get_graph()

    solver = Solver()
    colony = Colony(1, 3)
    sales = nSalesmen

    ans = solver.solve(G, colony, sales, start=(depotIndex + 1), limit=50, opt2=20)
    #print(sum(s.cost for s in ans), ans)
        
    paths = [[p[0] - 1 for p in s.path] + [s.path[-1][1] - 1] for s in ans]
    
    return {"routes": paths, "cost": sum(s.cost for s in ans)}
