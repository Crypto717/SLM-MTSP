...
def runGAImpl(nodes, depotIndex, nSalesmen):
    # Bank parameters
    nodes[0], nodes[depotIndex] = nodes[depotIndex], nodes[0]
    global atms_number
    atms_number = len(nodes) - 1        # ATM quantity
    global service_centers
    service_centers = 1     # service centers quantity
    global velocity
    velocity = 100          # 100 / hour
    global repair_time
    repair_time = 0         # 0.5 hour
    max_engi = nSalesmen            # maximum number of engineers in one service center
    ...
    for i in range(service_centers):
        for j in range(max_engi): #random.randint(1, max_engi)):
            engineers.append(i)
    engineers = np.array(engineers)
    #print('Engineers: {}'.format(engineers))
    global dist
    dist = np.zeros((atms_number+service_centers, atms_number))
    points_locations = np.array(nodes) #np.random.randint(0, 100, (service_centers+atms_number)*2)
    ...
    paths = population[np.argmin(fitness_result)]
    cost = 0
    for i in range(len(paths)):
        cost += dist[0][paths[i][0]]
        
        for j in range(len(paths[i])):
            if j == len(paths[i]) - 1:
                cost += dist[0][paths[i][j]]
            else:
                cost += dist[paths[i][j] + 1][paths[i][j + 1]]
            
            paths[i][j] += 1
            if paths[i][j] == depotIndex:
                paths[i][j] = 0
            
        paths[i].insert(0, depotIndex)
        paths[i].append(depotIndex)
    
    nodes[0], nodes[depotIndex] = nodes[depotIndex], nodes[0]
    
    return {"routes": paths, "cost": cost}